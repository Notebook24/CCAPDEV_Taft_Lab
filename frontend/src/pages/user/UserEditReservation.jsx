...
function UserEditReservation() {
  ...
  useEffect(() => {
    const resolve = async () => {
      try {
        const bRes = await fetch('http://localhost:3000/admin');
        ...
        const lRes = await fetch(`http://localhost:3000/admin/${building._id}/laboratories`);
  ...
  const fetchSeats = async (slotIdx, bId, lId) => {
    ...
    try {
      ...
      const url =
        `http://localhost:3000/user/reservation/${bId}/${lId}/seats` +
  ...
  const handleConfirm = async (e) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3000/user/reservation-history/${reservation.id}/edit`,
        {
          method: 'PUT',
