...
// Get the current time and return it in string format
...
function getInitialSlotIndex() {
    ...
    for (let i = 0; i < TIME_SLOTS.length; i++) {
        if (ct < TIME_SLOTS[i].end) return i; // Get the timeslots that are yet to end
    }
...
function AdminBuildingDashboard() {
    ...
    // For displaying admin navbar profile picture
    const [adminProfilePic, setAdminProfilePic] = useState(profileIcon);
    // For displaying student profile picture
    ...
    // Get the admin's profile picture for dispalying in the navbar
    useEffect(() => {
    ...
    // Get the laboratories in the specific building by calling the API in backend
    ...
    // Get the reservations in the specific building by calling API in backend
    ...
    // Get the students who reserved a laboratory recently in the spcific building
    async function fetchRecentStudents(buildingId, isInitialLoad = false) {
        ...
        try {
            ...
            // Get each student's profile picture
            const seen = new Set();
            const fetches = data
                .filter(s => {
    ...
    // Executes all get functions for fetcing laboratories, reservations, and recent students who reserved
    ...
    // Loads building dashboard data and refreshes the display every 60 seconds
    useEffect(() => {
    ...
    const today = getManilaToday(); // Get current date in Manila timezone

    // Initialize counters for dispalying data for reservtaion
    ...
    // Loop through reservations in a specific building to get needed data/statistics
    ...
    const firstAvailableSlotIndex = getInitialSlotIndex(); // Get the first time slot that is yet to end
    const activeSlot = TIME_SLOTS[slotIndex];

    // Navigate to previous time slot if not at first available slot
    ...
    // Calculates how many seats are taken in a lab for the active time slot
    ...
    // Returns unique recent students, removes duplicates by user ID
    ...
    // Navigates to the manage reservations page with data of building, lab, and timeslot
